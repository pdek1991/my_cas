...
def update_rps():
    ...
    file.write(str(osm_rps+device_rps+entitlement_rps))
    file.close()
...
logging.basicConfig(format='%(asctime)s.%(msecs)03d %(levelname)-8s %(message)s', datefmt='%d-%m-%Y:%H:%M:%S',
                    level=logging.DEBUG, handlers=[TimedRotatingFileHandler(filename=r'.\Logs\mycas.txt', backupCount=10, when='midnight', interval=1)])
...
if __name__ == '__main__':
    Timer(30, update_rps).start()
    app.run(host='192.168.56.115', port='5050')
